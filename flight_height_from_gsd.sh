...
# compute flight height f(desired GSD)
if [ $# -lt 2 ]; then
    echo $(basename $0) "camera GSD_cm " 1>&2
    exit 1
fi
GSD=$2
case $1 in
...
echo $F $PIX $GSD | awk '{ printf "DJI: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
;;
canon)
echo "------------------------------------------------------------------------"
F=4.5
...
HPIX=3000
PIX=1.549
echo $F $PIX $GSD | awk '{ printf "Canon: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
;;
sony)
echo "#------------------------------------------------------------------------"
F=16.0
...
echo $F $PIX $GSD | awk '{ printf "Sony alfa5000: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
;;
nikon)
echo "------------------------------------------------------------------------"
F=50.0
...
echo $F $PIX $GSD | awk '{ printf "NIKON D810: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
...
echo $F $PIX $GSD | awk '{ printf "iXa180: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
;;
vexel)
echo "#------------------------------------------------------------------------"
F=70
...
echo $F $PIX $GSD | awk '{ printf "Vexel: focal= %.2f mm   pixel= %.5f microns GSD= %d cm  FlightHeight %.2f m\n", $1,$2,$3, $3*10*$1/$2  }'
;;
lmk)
echo "#------------------------------------------------------------------------"
F=152.274
